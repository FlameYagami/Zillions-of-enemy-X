    ...
    public class AnalyseUtils
    {
        ...
        public static int AnalysePageCount(HtmlNodeCollection nodes)
        {
            ...
            var pageCountText = pageInnerText.Substring(
                ...
                pageInnerText.LastIndexOf("件", StringComparison.Ordinal) -
                pageInnerText.IndexOf("全", StringComparison.Ordinal) - 1);
        ...
        {
            return string.IsNullOrWhiteSpace(nodes[3].InnerText.Trim())
                ? ""
                : nodes[3].InnerText.Trim().Replace("&nbsp;", "");
        }
        ...
        public static string AnalyseRare(HtmlDocument frame)
        {
            try
            {
                ...
                return rareText.Substring(rareText.IndexOf("_", StringComparison.Ordinal) + 1,
                    rareText.LastIndexOf("_", StringComparison.Ordinal) -
                    rareText.IndexOf("_", StringComparison.Ordinal) -
                    1).ToUpper();
            }
            catch (Exception)
            {
                LogUtils.Write("罕贵解析失败");
                return StringConst.NotApplicable;
            }
        }
