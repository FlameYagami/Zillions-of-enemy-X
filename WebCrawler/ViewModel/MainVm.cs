...
namespace WebCrawler.ViewModel
{
    public class MainVm : BaseModel
    {
        ...
        public async void CmdCover_Click(object obj)
        {
            ...
            var sqlList = cardModels.Select(checkedModel => allNumberList.Contains(checkedModel.Number) ? GetUpdateSql(checkedModel) : GetAddSql(checkedModel)).ToList();
            //            var result = SqliteUtils.Execute(sqlList);
            //            BaseDialogUtils.ShowDialogAuto(result ? StringConst.CoverSucceed : StringConst.CoverFailed);
            sqlList.ForEach(sql => Console.WriteLine(sql));
        }

        private string GetAddSql(CardModel card)
        {
        ...
        private string GetUpdateSql(CardModel card)
        {
            var cost = card.Cost.Equals(-1) ? string.Empty : card.Cost.ToString();
            var power = card.Power.Equals(-1) ? string.Empty : card.Power.ToString();
        ...
        {
            if (0 == pageCount)
                return $"https://www.zxtcg.com/card/?fwcn=1&fwil=1&fwct=1&fwft=1&pn={PackValue}";
            else
                return $"https://www.zxtcg.com/card/?page={pageCount}&fwcn=1&fwil=1&fwct=1&fwft=1&pn={PackValue}";
        }
    }
}